...
def is_html_content(text):
    """
    Helper function to detect if text contains HTML content

    :param text: text to check for HTML content
    :return: boolean indicating if text contains HTML content
    """
...
def extract_text_from_html(html_content):
    ...
    # Remove HTML tags
    # First, replace <br>, <li>, <p>, <div>, <h1>-<h6> tags with newlines to preserve structure
    html_content = re.sub(r'<br\s*/?>|</li>|</p>|</div>|</h[1-6]>', '\n', html_content)
...
def key_or_default(data: dict, key: str, default_value=None):
    try:
        return data[key]
...
def extract_text(file_path, extension):
    ...
    if extension == '.pdf':
        ...
        # For HTML files, read the file and extract text
        ...
        # If the extension is not recognized, check if the content is HTML
        if isinstance(file_path, str):
            # If file_path is a string, it could be a file path or HTML content
            if os.path.exists(file_path):
                ...
                try:
                    ...
                    # If we can't read the file, return the file path
                    return file_path
            else:
